...
namespace VajaElektrika
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"Maksimalna je: {x4}");
            
            //5. izračunaj minimalno moč za ta datum
            var x5 = x2.Min(e => e.kW1 + e.kW2 + e.kW3);
            Console.WriteLine($"Minimalna je: {x5}");

            //6. izračunaj povprečno moč po urah za dan 18.8.2013
            var x6 = x2.GroupBy(e => e.ZapisČas.Value.Hour).OrderBy(e => e.Key).Select(e => new { Ura = e.Key, Moč = e.Average(a => a.kW1 + a.kW2 + a.kW3) });

            foreach (var x in x6)
            {
                Console.WriteLine($"{x.Ura}\t{x.Moč}");
            }
            ...
            foreach (var x in x7)
            {
                Console.WriteLine($"{x.Ura}\t{x.Četrt}\t{x.Moč}");
            }
